...
pygame.init()
pygame.mixer.music.load(dir_path + '/music.mp3')
...
def startingLocal(data,x,y): #takes in the user input for the tree initializing
...
def startTree(data):   #function that actually makkes the tree
    if not data.isDead:
        pass
    if data.startTheta > 120:
        data.startTheta = 120
    elif data.startTheta < 60:  #sets max angles in both directions
        data.startTheta = 60
    if data.fertile[0].x0-1 < data.start < data.fertile[0].x1+1: #if x in correct range
        new = branch(data.start,data.fertile[0].y0,data.endStart,data.startH,data.startTheta)
        data.branches.append(new)
        data.isDead = False
        data.inRangeF = False

def getPOS(data):   # sets the starting position for the tree, and finds the angle
...
def spreadDisease(data,branch): #spreads disease throughout tree
    if branch.disease > 0.5:
        for child in branch.children:
            child.disease  = min(1,child.disease+0.01)
        if branch.parent != None:   #different disease spread for child and parent
            parent = branch.parent
            parent.disease = min(1,parent.disease+.01*branch.disease)

def createStars(data): #makes the stars in a level
    l = []
    for i in range(data.win):
        l.append(star(data.height,data.width))
    return l

def resetLV(data):      #this is used to reset the level if the tree is dead
...
def findStart(data):    #finds the starting area
    start = False
    for y in range(data.height//4,data.height):
        for x in range(data.width):
            if start == False and data.rgbImage.getpixel((x,y)) == (1,1,1):
                ...
                X1 = x
                Y1 = y
    return Fertile(X0,Y0,X1,Y0-50)  #searches for a color line within the image

def moveStart(data,x,y):    #allows for the start to follow the mouse
    data.drawStuff[-1] = (['start',x,y,x+200,y+25])

def makeShadow(data):   #used for creating shadows
    func = data.drawStuff[-1][0]    #start contains the type of object it is
    dat = data.drawStuff[-1][1:]    #rest is data
    length = 1200
    theta = math.pi*(data.drawAngle)/180
    if func == 'circ':  #cretaes a polygon as its shadow
        x,y,color,rad = dat
    ...
    if func == 'start': #creates a line as its shadow
        x0,y0,x1,y1 = dat
        x2,y2 = x1-length*math.cos(theta),y1+length*math.sin(theta)
        x3,y3 = x0-length*math.cos(theta),y0+length*math.sin(theta)
        data.shadows.append([(x0,y0),(x1,y1),(x2,y2),(x3,y3)])
    ...
    if func == 'free':
        new = []
        l = 1
        dat = dat[:-1]  #last entry is color
        while l < length:
            new = []
            for (x,y) in dat:
                x1,y1 = x-l*math.cos(theta),y+l*math.sin(theta)
                new.append((x1,y1)) #creates a new polygon slightly shifted
            l += 1
            data.shadows.append(new)

def findLevels(data): #determine how many levels there are currently
    count = 0
    path = dir_path + '/Levels'
    for filename in os.listdir(path):
        count += 1
    return count

def applyWind(data): #causes the swaying of the tree
    if data.branches != []:
        if data.newAngle == -1:
            if data.wind == 1:
        ...
        if data.windApplied: #moves the branch further away from its initial
            if int(data.branches[0].theta) == int(data.newAngle):
                data.windApplied = False
            else:
                diff = data.newAngle-data.branches[0].theta
                if diff > 0:
                    data.branches[0].theta += 0.05
                else:
                    data.branches[0].theta -= 0.05
        else:       #brings the tree back to its initial
            if int(data.branches[0].theta) == int(data.startTheta):
...
def createLeafs(data): #creates leafs for the wind
    l = []
    for i in range(20):
...
################################################################################
                                #Draw functions#
################################################################################
def drawFertile(data,screen): #draws the starting area
...
def drawStars(data,screen): #draws the stars
    for star in data.stars:
...
def drawFallingBranches(data,screen): #draws any branches that are falling
...
def drawBranches(data,screen): #draws the other branches
...
def drawFlowers(data,screen): #draws all the flowers, also draws falling flowers
...
def drawCutter(data,screen): #draws cutting tool
    if len(data.points) > 1:
        pygame.draw.lines(screen,(BLACK),0,data.points,7)
        pygame.draw.lines(screen,(WHITE),0,data.points,5)

def drawMenu(data,screen): #draws the main mentu screen
...
def drawHelp(data,screen): #draws the menu button
...
def drawNext(data,screen): #draws the next button
...
def drawOptions(data,screen): #draws the options for the level editor
    if data.dColor == RED:
        color = BLACK
    else:
        color = WHITE
...
def drawMap(data,screen): #draws all the objects in the level editor, as you draw them
...
def drawShadows(data,screen): #makes shadows if none exist and then draws them
    if data.shadows == []:
...
def drawWin(data,screen): #tells the player that they have beaten the game
...
def drawLevels(data,screen): # draws all the current levels in the level selector
    count = 0
    path = dir_path + '/Levels'
    for filename in os.listdir(path):
...
def drawLeaf(data,screen): #draws the leafs that are falling
...
def mouseStart(data,x,y): #mousepressed for main menu
...
def mouseEdit(data,x,y): #mouse pressed for level editor
    if not data.drawing:
...
def mouseLev(data,x,y): #mouse pressed for level selector
    if data.width/2-data.w/2 < x < data.width/2+data.w:
        if  10 < y < 10 + data.h:
            data.splash = 'start'
    levs = []
    ...
    for i in range(1,16):
        X = 50*((i-1)%5 + 1) + 100*((i-1)%5)
        Y = 200+50*((i-1)//5)+100*((i-1)//5)
        path = "%d.png" %(i-1)
        if X < x < X+100:
            if Y < y < Y+100:
                if path in levs:
                    ...
                    data.wind = random.randint(-1,1)
                    break
def mouseWin(data,x,y): #mouse pressed to move onto next level
    if size(x,y,data.width//2,data.height//2) < 100:
        data.currentLv += 1
        string = dir_path + "/Levels/%d.png" %data.currentLv
        try:
...
def timerStart(data): # timer for the menu screen tree
...
def gameTimer(data): # timer that goes on for the game
    test = False
    for branch in data.branches:
        if not branch.cut:
            if branch.children != []:
...
def AlwaysTimer(data): #timer that is always running
...
def imageInit(data):
    imagePath = dir_path + "/Levels/%d.png" %data.currentLv
...
def mouseDrag(data,x,y): #creates the cutter or creates objects in editor
    if data.splash == False:
...
def redrawAllEdit(data,screen):
    ...
    if (not data.done):
        ...
        drawShadows(data,screen)
        drawMap(data,screen)
        pygame.image.save(screen,dir_path + "/Levels/%d.png" %data.numLevs)
        init(data)
...
def run():
    ...
    while data.playing:
        ...
        if data.splash != False:
            screen.fill(SUN)
            if data.splash == 'help':
                helpLV = pygame.image.load(dir_path + "/HelpScreen.png")
                screen.blit(helpLV, (0, 0))
            elif data.splash == 'preEdit':
                preEdit = pygame.image.load(dir_path + "/PreEdit.png")
